...
import { BellRing, Plus, Trash2, Loader2, ArrowUpRight, ArrowDownRight, CheckCircle2 } from 'lucide-react';
...
export default function AlertsTab({ prices, onAddSymbol }: AlertsTabProps) {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    // Determine if we are waiting for it to go ABOVE or BELOW
    const direction = currentPrice > 0 && target > currentPrice ? 'ABOVE' : 'BELOW';

    try {
      const res = await fetch('/api/alerts', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ symbol: newSymbol, targetPrice: target, direction }),
      });
